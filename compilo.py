    ...
    if expr.data == "variable":
        ...
        if expr.children[1] == "+":
            ...
            return f"""{e1}\npush rax\n{e2}\npush rbx\npop rax\npop rbx\n\
                ...
                cmp {type(exp1)} 2\nje s1\ns1: cmp {type(exp2)} 0\nje s+i\njne p+s\n\
                i+p: \njmp fin\n\
                i+s:\nlea rcx, {exp1.children[0].value}\nmov ebx, {exp1.children[0].value}\mov edx, 8\nmov rsi, rcx\nmov edi, ebx\nmov ebx, 0\ncall itoa\mov edx, {long1}\nmov ebx, {long2}\nadd edx, ebx\nmovsx rdx, ebx\nsub rdx, 1\nmov len_concat, rdx\nmovsx rdx, ebx\nmov r10, rdx\nmov r11d, 0\nmovsx rdx, ebx\nmov r8, rdx\nmov e9d, 0\ncdqe\nmov edx, 16\nsub rdx, 1\nadd rbx, rdx\nmov esi, 16\nmov edx, 0\ndiv rsi\nimul rbx, rbx, 16\nsub rsp, rbx\nmov rbx, rsp\nadd rbx, 0\nmov rax, rbx\nmov i, 0\njmp debut{compteur-3}\ndebut{compteur}:\nmov eax, i\ncdque\ncmp rbx, 6\nja debut{compteur-2}\nmov ebx, i\ncdqe\nmovzx ecx, [{str(e1)}+rbx]\nmov rdx, rax\nmov eax, i\ncdqe\nmov rdx+rbx, cl\njmp debut{compteur-1}\nfin{compteur}\ndebut{compteur-1}:\nadd i, 1\nfin{compteur-1}\n\\ndebut{compteur-3}:\nmov ebx, rax\ncmp eax, len_concat\njl debut{compteur}\nmov rsp rsi\nfin{compteur-3}\ndebut{compteur-2}:\nmov ebx, rax\ncdqe\nmovzx ecx, [{e2}+rbx]\nmov rdx, rax\nmov eax, i\ncdqe\nmov [rdx+rbx], cl\nfin{compteur-2}    
...
def compile_cmd(cmd):
    if cmd.data == "assignment":
        ...
        e2 = cmd.children[2]
        if type(e2) == "2" :
            e2 = f"{ord(e2.children[0].value)}"
        return f"mov [{v} - {e1}], {e2}"
    else:
        raise Exception ("Not Implemented")
...
prg = grammaire.parse("""main(X,Y) {X = "abcdef"; Y = 3; Z = X.charAt ( Y ); return(Z);}""")
print(compile(prg))
def gamma_expr(expr):
    if expr.data == "nombre":
